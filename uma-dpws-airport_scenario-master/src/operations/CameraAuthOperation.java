...
public class CameraAuthOperation extends Operation {
	
	/**
	 * Constructor
	 */
	public CameraAuthOperation() {
		super("CameraAuthOperation", new QName("BasicServices", DeviceCamera.namespace));

		// create new Element called "name" (just a simple one in this case)
		Element nameElem = new Element(new QName("auth", DeviceCamera.namespace), SchemaUtil.TYPE_STRING);

		// set the input of the operation
		setInput(nameElem);

		// create new element called "reply"
		Element reply = new Element(new QName("authResponse", DeviceCamera.namespace), SchemaUtil.TYPE_STRING);
	...
	public ParameterValue invokeImpl(ParameterValue parameterValue, CredentialInfo credentialInfo) throws InvocationException, CommunicationException {
		...
		ParameterValueManagement.setString(result, "reply", "User " + name + " authenticated");
