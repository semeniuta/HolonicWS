...
public class VideoStopOperation extends Operation {
	/**
	 * Constructor
	 */
	public VideoStopOperation() {
		super("VideoStopOperation", new QName("BasicServices", DeviceVideo.namespace));

		// create new Element called "name" (just a simple one in this case)
		Element nameElem = new Element(new QName("stop", DeviceVideo.namespace), SchemaUtil.TYPE_STRING);

		// set the input of the operation
		setInput(nameElem);

		// create new element called "reply"
		Element reply = new Element(new QName("stopResponse", DeviceVideo.namespace), SchemaUtil.TYPE_STRING);
	...
	public ParameterValue invokeImpl(ParameterValue parameterValue, CredentialInfo credentialInfo) throws InvocationException, CommunicationException {
		...
		ParameterValueManagement.setString(result, "reply", "Video is stopped");
